function calculateDayInYear(date) {
  ...
  function validMonth(month) {
    return month && month >= 1 && month <= 12; //changed month < 12 to  <=
  }

  function validDay(month, day) {
    return day && day >= 1 && day <= DAYS_IN_MONTH[month - 1]; //changd day < DIM to <=
  }
  ...
  function daysInFeb(year) {
    //added if/else statement to check against isLeapYear
    if (isLeapYear(year)) {
      return 29
    } else {
    return 28
